...
public sealed partial class RdbmsHtmlGeneratorViewModel : ObservableObject
{
    ...
    public string CurrentDatabaseName => SelectedSource == DatabaseSourceType.Postgres
        ? (string.IsNullOrWhiteSpace(PostgresConnectionSettings.Database) ? "Rdbms.HtmlGenerator" : PostgresConnectionSettings.Database)
        : (string.IsNullOrWhiteSpace(OracleDatabaseName) ? "Rdbms.HtmlGenerator" : OracleDatabaseName);
