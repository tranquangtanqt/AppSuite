...
```powershell
dotnet run --project Modules\Mcf.Screen.HtmlGenerator\Mcf.Screen.HtmlGenerator.csproj
```

hoặc mở `Mcf.Screen.HtmlGenerator.csproj` riêng trong Visual Studio, đặt Startup Project, F5.
...
liệu dạng JSON vào 1 thẻ `<script type="application/json">` (giống cách Mcf.DbDef.HtmlGenerator/E đã làm), giống nhau
...
- Đường dẫn dữ liệu resolve theo `AppContext.BaseDirectory` của chính tiến trình Mcf.Screen.HtmlGenerator (giống
  Mcf.DbDef.HtmlGenerator/E) - `Data\Config\config.xml`, `Data\Database\01_画面説明書.db`, `Data\Database\Html\` tự tạo lúc
