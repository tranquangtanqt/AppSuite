...
namespace Mcf.DbDef.HtmlGenerator.ViewModels;

/// <summary>
/// Drives the three-step pipeline: read Data\Excel\*.xlsm -> save to Data\Database\Mcf.DbDef.HtmlGenerator.db ->
/// export Data\Database\Mcf.DbDef.HtmlGenerator.html. Mirrors ModuleC's SearchViewModel style (CommunityToolkit.Mvvm,
/// manually constructed - no DI container).
/// </summary>
public sealed partial class McfDbDefHtmlGeneratorViewModel : ObservableObject
{
    private readonly ExcelDbDefImporter _importer = new();
    private readonly McfDbDefHtmlGeneratorDatabase _database = new();
    ...
    public McfDbDefHtmlGeneratorViewModel()
    {
        CanExportHtml = _database.Exists;
        var expectedHtmlPath = Path.Combine(AppContext.BaseDirectory, "Data", "Database", "Mcf.DbDef.HtmlGenerator.html");
