...
/// <summary>
/// Persists the imported DB-dictionary into a local SQLite file at Data\Database\ModuleD.db, next to
/// the module executable - same pattern as ModuleB's SavedFolderRepository (plain ADO.NET, no ORM).
/// </summary>
public sealed class ModuleDDatabase
{
    private readonly string _connectionString;

    public string DatabasePath { get; }

    public ModuleDDatabase()
    {
        var dataDirectory = Path.Combine(AppContext.BaseDirectory, "Data", "Database");
        Directory.CreateDirectory(dataDirectory);
        DatabasePath = Path.Combine(dataDirectory, "ModuleD.db");
