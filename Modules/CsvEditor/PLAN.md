# CsvEditor: CSV/TSV Editor cho AppSuite
...
`.claude\skills\appsuite-dev\SKILL.md`), đây là **1 module mới độc lập "CsvEditor"**, chỉ
...
(chỉ MainLauncher dùng) - ViewModel/Service được `new` thủ công. CsvEditor theo đúng convention thực tế
...
`CsvEditorViewModel.ViewRows` (không phải `Document.Rows`) là thứ DataGrid bind vào - chứa cùng
...
`AppSuite.sln` (project `CsvEditor` GUID `34DBF542-F78F-4E48-8C17-40626EF024CF`),
...
12. Context menu: Copy nhiều dòng đã chọn dán được vào Excel (TSV), Paste nhiều ô, Delete, Insert Row,
    Duplicate Row.
13. `dotnet build AppSuite.sln` - CsvEditor không ảnh hưởng project khác; `Sync-Modules-Dev.ps1` copy
    đúng `CsvEditor.exe` cạnh `MainLauncher.exe`.
