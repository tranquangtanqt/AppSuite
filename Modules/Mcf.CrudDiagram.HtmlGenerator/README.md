...
```powershell
dotnet run --project Modules\Mcf.CrudDiagram.HtmlGenerator\Mcf.CrudDiagram.HtmlGenerator.csproj
```

hoặc mở `Mcf.CrudDiagram.HtmlGenerator.csproj` riêng trong Visual Studio, đặt Startup Project, F5.

## Khác Mcf.Screen.HtmlGenerator ở đơn vị "1 record"

Mcf.Screen.HtmlGenerator là "1 file = 1 màn hình" (nhiều sheet/file). Ở Mcf.CrudDiagram.HtmlGenerator, **1 sheet không nằm trong danh sách
...
3. **"2. Xuat HTML"** - đọc bảng `Logics`, sinh `Data\Database\Html\02_CRUD図.html` (nhúng sẵn danh
   mục dạng JSON, không `fetch()` file ngoài - lý do giống hệt Mcf.Screen.HtmlGenerator, xem README Mcf.Screen.HtmlGenerator) +
...
phải (`<iframe name="contentFrame">`), giống cơ chế Mcf.Screen.HtmlGenerator.

## Bộ dịch Excel → HTML: 1 semantic parser duy nhất cho mọi sheet

Khác Mcf.Screen.HtmlGenerator (phải có 3 parser riêng theo từng loại sheet + fallback lưới chung vì mỗi sheet trong
...
liệu, theo đúng triết lý "never silently lose content" đã áp dụng ở Mcf.Screen.HtmlGenerator.
...
- Đường dẫn dữ liệu resolve theo `AppContext.BaseDirectory` của chính tiến trình Mcf.CrudDiagram.HtmlGenerator (giống
  Mcf.Screen.HtmlGenerator) - `Data\Config\config.xml`, `Data\Database\02_CRUD図.db`, `Data\Database\Html\` tự tạo lúc
