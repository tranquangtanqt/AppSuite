...
## Build/publish riêng ModuleF (không build cả AppSuite)

Kiểm tra lỗi biên dịch nhanh (không publish, không copy file):

```powershell
dotnet build Modules\ModuleF\ModuleF.csproj -c Release
```

Publish đúng layout deploy (`Application\Modules\ModuleF\...`) mà không đụng tới MainLauncher/7
module còn lại - `build\Publish-AppSuite.ps1` hỗ trợ sẵn `-Targets`:

```powershell
powershell -ExecutionPolicy Bypass -File .\build\Publish-AppSuite.ps1 -Configuration Release -Runtime win-x64 -Targets ModuleF
```
