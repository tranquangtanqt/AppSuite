# ModuleG: Tài liệu màn hình (画面説明書) Excel → SQLite → HTML
...
dạng bảng dữ liệu phẳng như DBDef của ModuleD - là tài liệu định dạng phức tạp (merge cell dày đặc,
màu nền, ảnh nhúng, đoạn văn bản dài dạng gạch đầu dòng).

Người dùng muốn cùng mô hình pipeline ModuleD/E (Excel → cache SQLite → xuất HTML duyệt được), với
UI có 2 ô tìm kiếm bên trái: 1 ô tìm theo **mã màn hình** (vd `MSBBP1210`), 1 ô tìm theo **nội dung**.
Kiến trúc AppSuite cấm module phụ thuộc lẫn nhau nên đây là **module độc lập mới "ModuleG"**.

### 2 quyết định đã chốt với người dùng trước khi thiết kế

1. **Dịch sheet → HTML bằng generic grid renderer** (không parse ngữ nghĩa từng mục như ModuleD) -
...
2. **Xuất nhiều file**: mỗi màn hình → 1 file `.html` riêng (+ ảnh nhúng lưu file rời cạnh đó) thay vì
   nhúng tất cả (617 màn hình + ảnh) vào 1 file HTML tự chứa như ModuleD/E - tránh 1 file khổng lồ,
...
### 1. Khung module (giống ModuleD/E)

`Modules\ModuleG\{App.xaml(.cs), MainWindow.xaml(.cs), Assets/*, Package.appxmanifest, app.manifest,
ModuleG.csproj, README.md, PLAN.md}`. `ModuleG.csproj`: `ProjectReference` chỉ tới `Common`;
`PackageReference` giống hệt ModuleD (`Microsoft.Windows.SDK.BuildTools`, `Microsoft.WindowsAppSDK
...
(đã dùng ổn định trong ModuleD). Không có DI container, theo đúng convention thực tế của Module.

### 2. Cấu hình thư mục nguồn (giống ConnectionSettingsStore của ModuleE)

Thư mục nguồn (`D:\Projects\...\01_画面説明書`) nằm ngoài repo, quá lớn để bundle như
`Data\Excel\*.xlsm` của ModuleD - user chọn qua nút **"Thiet lap thu muc nguon"** mở dialog (TextBox
...
`AppContext.BaseDirectory`-relative như ModuleE) để tự điền lại lần sau.

### 3. Models (`Modules\ModuleG\Models\`)
...
- **`ModuleGDatabase.cs`** - cùng pattern ADO.NET thuần (`Microsoft.Data.Sqlite`) như
  `ModuleDDatabase`/`ModuleEDatabase`: 1 bảng `Screens` (`Id, ScreenCode, ScreenName, DocNumber,
  Revision, SourceFile, SearchText, HtmlFileName`), `ReplaceAllAsync` drop+create+insert lại toàn bộ
  mỗi lần chạy import, file `Data\Database\ModuleG.db`.
- **`HtmlIndexGenerator.cs`** - đọc toàn bộ `Screens` từ DB, ghi `Data\Database\Html\index.html`
  ...
  style/pattern menu trái của ModuleD) + `Data\Database\Html\manifest.json`
  ...
  **Sửa lại lúc code so với bản plan ban đầu**: `index.html` **không** `fetch()` `manifest.json` lúc
  ...
  `<script type="application/json">` ngay trong `index.html` (giống hệt kỹ thuật ModuleD/E đã dùng),
...
### 5. ViewModel + Giao diện (`ModuleGViewModel`, `MainWindow.xaml(.cs)`)

Giống hệt `ModuleDViewModel`/`ModuleEViewModel` (không DI, `AppendLog` qua `DispatcherQueue`):
...
(`ScreenDocImporter` + `ModuleGDatabase.ReplaceAllAsync` trên `Task.Run`, log theo từng file/617) →
**"2. Xuat HTML"** (`HtmlIndexGenerator`) → **"Mo file HTML"** (mở `index.html` bằng trình duyệt mặc
định). ProgressBar/StatusText/Log giữ nguyên style ModuleD.
...
- Import 617 file có ảnh nhúng sẽ chậm hơn nhiều so với ~10 file Excel nhỏ của ModuleD - chấp nhận
  chạy 1 lần rồi cache, không cần streaming/parallel hoá thêm (giữ đơn giản như ModuleD).
...
cách này gắn chặt vào format export riêng của BE đó. ModuleG đọc thẳng workbook bằng EPPlus (đã có
...
Đã build `ModuleG.csproj` + `AppSuite.sln` sạch, chạy thử qua harness console tạm (ngoài UI, dùng lại
...
1. `dotnet build Modules\ModuleG\ModuleG.csproj -p:Platform=x64` - build sạch.
...
5. `dotnet build AppSuite.sln` - ModuleG không ảnh hưởng ModuleA-F/MainLauncher/Common.
