# Mcf.DbDef.HtmlGenerator: Từ điển dữ liệu từ Excel "DBDef" (Excel → SQLite → HTML)
...
1. **"1. Doc Excel → SQLite"** - `Services\ExcelDbDefImporter` (EPPlus 4.5.3.3) đọc toàn bộ
   `Data\Excel\*.xlsm`, ghi vào `Data\Database\Mcf.DbDef.HtmlGenerator.db` qua `Services\McfDbDefHtmlGeneratorDatabase`
...
1. `dotnet build Modules\Mcf.DbDef.HtmlGenerator\Mcf.DbDef.HtmlGenerator.csproj` - build sạch.
