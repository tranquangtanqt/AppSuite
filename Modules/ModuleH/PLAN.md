# ModuleH: CRUD図 Excel → SQLite → HTML

## Bối cảnh

`D:\Projects\202607_MCF7\02_CRUD図` chứa 2 file `.xlsx` CRUD図 (ma trận Create/Read/Update/Delete:
logic nào dùng bảng/logic nào, dùng ra sao). Người dùng muốn 1 module tương tự ModuleG (Excel →
...
- **Khác ModuleG về đơn vị "1 record"**: ModuleG là "1 file = 1 màn hình (nhiều sheet)". Ở đây
...
- Layout dữ liệu (block ID + object rows với cờ C/R/U/D) đồng nhất tuyệt đối qua mọi sheet đã lấy
  mẫu → chỉ cần **1 semantic parser duy nhất**, khác hẳn ModuleG phải có 3 parser theo loại sheet.

## Thiết kế

Scaffold copy nguyên cấu trúc `Modules\ModuleG` sang `Modules\ModuleH` (đổi namespace), giữ đúng 3
...
`Data\Database\Html\02_CRUD図.html` (giữ quy ước đặt tên theo tên thư mục nguồn, giống ModuleG).
...
### Các thành phần khác - mirror 1:1 pattern ModuleG
...
- `Services\SourceFolderSettingsStore.cs` - copy nguyên từ ModuleG.
...
- `ViewModels\ModuleHViewModel.cs`, `MainWindow.xaml(.cs)` - mirror `ModuleGViewModel` không đổi gì
  về luồng (Import/ExportHtml/OpenHtml commands + log panel).
- `ModuleH.csproj` **không có SkiaSharp** (không có sơ đồ shape/connector nào cần vẽ lại như ModuleG).
...
- `dotnet build Modules\ModuleH\ModuleH.csproj -p:Platform=x64` build sạch.

Còn lại (đã làm nhưng chưa tự kiểm chứng bằng UI thật, cần user xác nhận): mở `ModuleH.csproj` F5,
