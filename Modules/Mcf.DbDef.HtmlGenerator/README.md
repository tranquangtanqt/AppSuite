...
```powershell
dotnet run --project Modules\Mcf.DbDef.HtmlGenerator\Mcf.DbDef.HtmlGenerator.csproj
```

hoặc mở `Mcf.DbDef.HtmlGenerator.csproj` riêng trong Visual Studio, đặt Startup Project, F5.

## Chức năng - pipeline 3 bước

1. **"1. Doc Excel → SQLite"** - đọc toàn bộ `Data\Excel\*.xlsm` (workbook "DBDef" tiếng Nhật) bằng
   EPPlus 4.5.3.3, rồi lưu vào SQLite tại `Data\Database\Mcf.DbDef.HtmlGenerator.db` (`Services\McfDbDefHtmlGeneratorDatabase`,
...
2. **"2. Xuat HTML"** - đọc lại `Mcf.DbDef.HtmlGenerator.db`, sinh 1 file HTML tĩnh, tự chứa (data nhúng dạng JSON,
   JS thuần, không phụ thuộc CDN/mạng) tại `Data\Database\Mcf.DbDef.HtmlGenerator.html` (`Services\HtmlReportGenerator`).
...
- Đường dẫn dữ liệu resolve theo `AppContext.BaseDirectory` của chính tiến trình Mcf.DbDef.HtmlGenerator (giống
  ModuleB/ModuleC) - `Data\Excel\*.xlsm` được copy cạnh exe qua `CopyToOutputDirectory=PreserveNewest`
  trong `Mcf.DbDef.HtmlGenerator.csproj`, không phụ thuộc ai khởi động tiến trình (VS, `dotnet run`, hay
